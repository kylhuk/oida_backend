...
| --- | --- | --- | --- | --- |
| `meta.source_registry` | source definitions, governance, and parser routing | `ReplacingMergeTree(version)` with mirrored `record_version` writes | `(source_id)` | `schema_version`, `record_version`, `api_contract_version`, `updated_at`, `attrs`, `evidence`, `requests_per_minute`, `burst_size`, `retention_class`, `disabled_reason`, `disabled_at`, `disabled_by`, `review_status`, `review_notes`, `auth_config_json`, `backfill_priority`, `attribution_required` |
...
- New registries should not use generic `version` column names; use `record_version`. `meta.source_registry` keeps the legacy `version` column only for backward compatibility with the pre-`0004` seed path, but seed evolution must mirror `version` and `record_version` so compatibility views can read latest state with `FINAL`.
...
- Source seed evolution stores a stable `seed_checksum` inside `attrs`; unchanged seeds must not emit new versions, while changed seeds insert a higher `record_version` row and preserve operational kill-switch state.
