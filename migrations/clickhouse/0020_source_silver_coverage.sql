CREATE VIEW IF NOT EXISTS silver.v_source_terminal_catalog AS
...
CREATE VIEW IF NOT EXISTS meta.source_silver_coverage AS
WITH
in_scope AS (
...
FROM (
	SELECT
		...
		multiIf(
			...
			promote_profile IN ('promote:geopolitical', 'promote:safety_security'), 'table',
			'none'
		) AS terminal_kind,
		multiIf(
...
LEFT JOIN (
	SELECT
		source_id,
		greatest(coalesce(o.last_observed_at, toDateTime64(0, 3, 'UTC')), coalesce(e.last_event_at, toDateTime64(0, 3, 'UTC')), coalesce(t.last_track_at, toDateTime64(0, 3, 'UTC'))) AS last_any_silver_at,
		coalesce(o.rows_observed, 0) AS rows_observed,
		coalesce(e.rows_event, 0) AS rows_event,
		coalesce(t.rows_track, 0) AS rows_track
	FROM in_scope
	LEFT JOIN observation_landing AS o USING (source_id)
	LEFT JOIN event_landing AS e USING (source_id)
	LEFT JOIN track_landing AS t USING (source_id)
) AS silver_rollup ON silver_rollup.source_id = s.source_id
ARRAY JOIN [
	multiIf(
...
[
	multiIf(
		...
		s.promote_profile = 'promote:catalog', c.last_catalog_at,
		null
	)
] AS last_silver_at;
